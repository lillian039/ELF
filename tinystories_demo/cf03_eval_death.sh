#!/usr/bin/env bash
# CF model evaluation chain, death: waits for elf_cf03_k64/checkpoint_75000,
...
D=/mnt/faster3/lc2762/elf_cf03_k64
...
echo "=== $(date '+%F %T') CF03 probe ==="
python3 src/eval_superposition.py --config ${D}/config.yml \
  --checkpoint_path ${D}/checkpoint_75000 \
  2>&1 | tee tinystories_demo/logs/cf03_sup.log | grep -aE "SUPERPOSITION_SUMMARY" || true

echo "=== $(date '+%F %T') CF03 natvar ==="
python3 src/eval_natural_variation.py --config ${D}/config.yml \
  --checkpoint_path ${D}/checkpoint_75000 --out paper/natvar_cf03.json \
  2>&1 | tee tinystories_demo/logs/natvar_cf03.log | grep -aE "NATVAR_SUMMARY" || true

echo "=== $(date '+%F %T') CF03 pairs12 ==="
python3 src/eval_leakage_pairs.py --config ${D}/config.yml \
  --checkpoint_path ${D}/checkpoint_75000 \
  --seeds 5 --samples-per-alpha 24 --targets extended2 --decon all \
  --out paper/pairs12_cf03.json \
  2>&1 | tee tinystories_demo/logs/pairs12_cf03.log | grep -aE "LEAKPAIR_SUMMARY" || true

echo "=== $(date '+%F %T') CF03 terminal-window trajectory ==="
python3 src/eval_leakage_trajectory.py --config ${D}/config.yml \
  ...
  --out paper/traj_cf03.json \
  2>&1 | tee tinystories_demo/logs/traj_cf03.log | grep -aE "TRAJ_SUMMARY|WINDOW_SUMMARY" || true

echo "CF03 EVAL CHAIN DONE $(date '+%F %T')"
